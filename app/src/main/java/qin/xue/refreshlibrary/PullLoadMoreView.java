...
//    private NestedScrollView nestedScrollView;
    ...
    private PointView pointView;//小圆点
...
//        nestedScrollView = (NestedScrollView) getChildAt(0);
        ...
        LayoutParams layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, UiUtils.dipToPx(50));
        ...
        topLayout.addView(headLayout,new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, UiUtils.dipToPx(500)));
        topLayout.addView(pointView, layoutParams);
        addView(topLayout, 0);
    }
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE://滑动
                ...
                if (abs > scaledTouchSlop) {
                    mTouchEvent = true;//nestedScrollView.getScrollY() == 0;
...
//                S.s("scaledTouchSlop:"+scaledTouchSlop+" "+abs);
                if (abs > scaledTouchSlop) {
                    mTouchEvent = true;
                    if (scrollYValue > 0) {
//                        if (nestedScrollView.getScrollY() == 0) {
                            if (stateView == VIewState.CLOSE) {
                                if (mPaddingTop < -subViewHeight*0.8f) {//向下滑动但是头部空间没完全显示
...
//                        }
    ...
    private void dotHideAnim() {
        ViewPropertyAnimator alpha = pointView.animate().alpha(0f);
