...
public class GalleryLayoutManagerNew extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mChildHeight == 0 || mChildWidth == 0) {
            ...
            mChildWidth = getDecoratedMeasuredWidth(firstView)/2;
            mChildHeight = getDecoratedMeasuredHeight(firstView)/2;
        ...
        fill(recycler, state);
        //S.s( "循环 onLayoutChildren: " + mLastRecyclePosition+",childcount:"+getChildCount() );
...
//        S.s( "[ "+dx+" ]fill() called with: state = [" + state + "]");
//        S.s("finger to " + (dx == 0 ? "stopped" : (dx > 0 ? "left" : "right")));
        //step 1 :回收越界子View
        ...
        if (dx >= 0) {
            ...
            if (mLastRecyclePosition != -1) {
                //S.s( "循环fill() mLastRecyclePosition: " + mLastRecyclePosition+",childcount:"+getChildCount() );
            ...
            for (int i = startPos; i < itemCount; i++) {
                //S.s( "循环add() called with: i = [" + i+",childcount:"+getChildCount() );
                //如果左边界已经大于屏幕可见(考虑offset)
                if (left - dx > getWidth() - getPaddingRight()) {
                    //S.s( "循环break() called with: i = [" + i +",childcount:"+getChildCount() );
                    break;
                }
...
/*        float fraction = dx * 1.0f / 45;
        ...
        float distance = parentMiddle - childMiddle;
        float fraction = distance / (getWidth() / 2f);
...
//        rotation(child, fraction);
    ...
    private void rotation(View child, float fraction) {
        if (view == null || view == child) {
            view = child;
            S.s("fraction:" + fraction + "        " + getPosition(child));
        }
        child.setPivotX(fraction<0?0:child.getMeasuredWidth());//child.getMeasuredWidth());
        child.setPivotY(child.getMeasuredHeight()/2f);
        child.setRotationY(-30 * fraction);
        child.setTranslationZ(10 * (1 - Math.abs(fraction)));
            ...
            for (int i = childCount - 1; i >= 0; i--) {
                ...
                if (dx > 0) {
                    ...
                    if (getDecoratedRight(child) - dx < getPaddingLeft()) {
                        //S.s( "循环 删除 () called with: getPosition(child) = [" + getPosition(child) + "], mLastRecyclePosition = [" + mLastRecyclePosition + "], dx = [" + dx + "]");
                        //逆序的 所以取最大的
                        mLastRecyclePosition = Math.max(mLastRecyclePosition, getPosition(child));
                        S.s("removeAndRecycleView: " + i);
